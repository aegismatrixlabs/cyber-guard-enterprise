...
from cyberguard.core.database import engine, Base, SessionLocal, get_db
...
def startup_event():
    ...
    if not db.query(User).filter(User.is_super_admin == True).first():
        db.add(User(username="superadmin", email="akeser3401@gmail.com", hashed_password=get_password_hash("super123"), is_super_admin=True))
        db.commit()
        print("✅ Süper Admin oluşturuldu: superadmin / super123")
    
    # 2. Normal Admin oluştur (E-posta eklendi)
    if not db.query(User).filter(User.username == "admin").first():
        admin_user = User(
            username="admin", 
            email="akeser3401+admin@gmail.com",  # TEST AMAÇLI E-POSTA EKLENDİ
...
# --- AŞAMA C: CORS (Çapraz Kaynak Paylaşımı) ve Health Check ---
from fastapi.middleware.cors import CORSMiddleware

# Tüm originlere izin ver (Geliştirme aşaması için)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def health_check():
    return {"status": "ok", "message": "CyberGuard Enterprise sistemi sorunsuz çalışıyor."}
